...
const makeSearchUrl = semantic => {
  ...
  return "/api/v1/contents?"
      ...
      + `${filter.location ? `&filter[location]=${filter.location.lat},${filter.location.lng}` : ''}`
      + `&page[offset]=${semantic.page.offset || 0}&page[limit]=${semantic.page.limit || 5}&sort=${semantic.sort || 'recommended'}&currency=${semantic.currency || 'JPY'}&locale=${(semantic.langs && semantic.langs[0]) || 'en'}&langs=${(semantic.langs && semantic.langs.join(',')) || 'en'}&fields[contents]=title,description,latitude,longitude,price`;
};
